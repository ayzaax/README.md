[![Hackathons](https://img.shields.io/badge/Hackathons-80%2B-brightgreen?style=flat-square)](https://github.com/ayzaax)
[![Followers](https://img.shields.io/github/followers/ayzaax?label=Followers&style=flat-square)](https://github.com/ayzaax)

## About
👋 Hola, soy Aylin. Soy desarrolladora apasionada por crear soluciones innovadoras. Me dedico a proyectos con enfoque en desarrollo web, aplicaciones móviles y tecnologías modernas.

- 📍 Location: [Tu Ciudad]
- 🎓 Education: [Tu Universidad o Institución]
- 📅 Coding Since: [Año que comenzaste a programar]

## Featured Projects
- 🎲 [Nombre del Proyecto 1] — [Breve descripción del proyecto.] ([Lenguaje]) — github.com/ayzaax/REPO_1
- 🛠️ [Nombre del Proyecto 2] — [Breve descripción del proyecto.] ([Lenguaje]) — github.com/ayzaax/REPO_2
- 🤖 [Nombre del Proyecto 3] — [Breve descripción del proyecto.] ([Lenguaje]) — github.com/ayzaax/REPO_3

## Tech & Interests
- 🧰 Languages: JavaScript, Python, TypeScript
- 🔭 Focus: Web Development, Clean Code, Open Source
- ❤️ Interests: Innovation, Problem-solving, Continuous Learning

---
*Este perfil fue generado usando GitHub Copilot en un hackathon de MLH (Major League Hacking). Encuentra eventos cerca de ti en [mlh.io/events](mlh.io/events).*
